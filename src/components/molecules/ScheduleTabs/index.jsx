...
const ScheduleTabs = () => {
  ...
  const { params } = route;
  const { data: theater } = useScheduleDetail(params.item._id);

  const routes =
    theater?.setlist.originalName === "Ramadhan Event"
      ? [
  ...
  const renderTabs = SceneMap({
    member: () => <Members members={theater?.memberList} />,
    songs: () => <SongsSetlist songs={theater?.setlist.songs} />,
    ticket: () => <Ticket ticket={theater} />,
  });
